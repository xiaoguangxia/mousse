...
Page({
  ...
  saveGwc(e){
    let that = this;
    // 加入购物车接口
    http.postData('/api/project/add_cart', {
  ...
  creatArticlePoster() {
    ...
    // share_text = that.data.setMealObj.share_text.length>36?share_text+'...':share_text;
    let texts = [
      {
        x: 30,
        y: 660,
        text: that.data.setMealObj.name,
        fontSize: 24,
        lineHeight:36,
        color: '#141414',
        width: 520,
  ...
  onLoad: function (options) {
    ...
    if(options.id){
      // 请求后台套餐详情接口
      http.postData('/api/project/info', {
        id:options.id
      }, (res) => {
        // 
        if(res.data){
